...
class Model(object):
    ...
    def attention3D(self, x, ch, dch=8, sn=True, name='attention'):
        ...
        gamma = tf.compat.v1.get_variable(name+"gamma", [1], initializer=tf.constant_initializer(0.0))
    ...
    def style_layer_loss(self, a, x):
        ...
        # N = c.value
        N = c
        A = self.gram_matrix(a, b, M, N)
        print(A.get_shape())
        X = self.gram_matrix(x, b, M, N)
    ...
    def build_model(self):
        ...
        '''D_loss'''
        eps = tf.compat.v1.random_uniform([self.batchSize, 1, 1, 1, 1], minval=0., maxval=1.)
